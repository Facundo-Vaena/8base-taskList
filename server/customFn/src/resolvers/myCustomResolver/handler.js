...
import gql from "graphql-tag";
 ...
 module.exports = async (event, ctx) => {
     let response = null;
     try {
         response = await ctx.api.gqlRequest(COMPLETE_TODO_RESOLVER, {id: event.data.id, text: event.data.text, completed: event.data.completed})
     } catch (error) {
         return { data: {success: false, messagge: "Something went wrong" } }
     }
     return { data: { success: true, messagge: "Task Updated" } }
